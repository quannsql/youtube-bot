...
OPENAI_TEXT_LONG_FORM_MAX_OUTPUT_TOKENS=48000
OPENAI_TEXT_ATTEMPTS=3
OPENAI_TEXT_RETRY_BACKOFF_SECONDS=5
OPENAI_TEXT_CONNECT_TIMEOUT_SECONDS=30
OPENAI_TEXT_READ_TIMEOUT_SECONDS=600
BRAVE_SEARCH_API_KEY=your_optional_brave_key
BRAVE_WEB_IMAGES_PER_SHORT=4
BRAVE_WEB_IMAGES_PER_LONG_FORM=28
...
Video dài chạy end-to-end trong một lượt: lập kế hoạch → tạo audio → co/giãn scene theo audio → thêm mốc chương vào mô tả → chuẩn bị visual (mặc định tối đa 28 ảnh thật qua `BRAVE_WEB_IMAGES_PER_LONG_FORM`, ưu tiên scene có `search_query`, + 14 ảnh AI qua `LONG_FORM_AI_IMAGES`; nếu tìm ảnh thật hụt thì ngân sách AI tự nới tới `LONG_FORM_AI_IMAGES_MAX` thay vì lặp lại hình cũ) → render (chỉ chèn logo, không chèn video overlay góc) → tạo thumbnail 16:9 → upload lên YouTube. Thumbnail được ghép bằng Pillow ở 1280x720: nền thiết kế riêng (ảnh chủ đề làm mờ + quầng sáng nhấn + vignette) + 1-3 ảnh THẬT đặt trong khung viền trắng nét cọ, nghiêng và chồng nhau theo bố cục nghệ thuật, lớp phủ tối chuyển sắc bên trái, badge đỏ nét cọ theo nhóm chủ đề, rồi headline in hoa TRẮNG KHÔNG viền/bóng — những từ quan trọng (chủ thể của video) được tô nền đỏ kiểu vệt cọ, hơi nghiêng. Chữ càng dài thì cỡ chữ tự co (tối đa 4 dòng) để luôn nằm gọn trong cột trái và không đè lên khung ảnh. Headline lấy từ `thumbnail_text` chỉ khi nó đủ dài (≥3 từ) và có nhắc tên chủ thể; nếu nó là mẩu teaser khó hiểu ("No Fuel", "One Road?") thì bot dùng thẳng title để người xem hiểu được nội dung video. `thumbnail_text` nay yêu cầu 3-6 từ, ≤34 ký tự, tiếng Anh dễ hiểu, nêu chủ thể và điều video giải thích — không còn là teaser 2-4 từ. Thứ tự lấy ảnh: tìm ảnh thật mới qua Brave → Wikimedia theo chủ thể và các `search_query` rải đều video → ảnh Wikimedia đã giữ lại → các scene đã dùng ảnh thật → frame trích từ video. Nếu mọi cách tạo thumbnail đều lỗi, video Long vẫn được upload mà không có custom thumbnail thay vì làm hỏng cả job. Bot chỉ upload `thumbnail.jpg` cho video Long; Short không dùng custom thumbnail. Voice được dùng đúng một lần; video tự khớp với thời lượng voice nên không có đoạn im lặng cuối và không cần tiêu voice/ảnh lần hai chỉ để sửa thời lượng. Nếu Railway bị restart sau khi audio và đủ số ảnh đã tạo, lượt chạy Long kế tiếp sẽ tự tiếp tục job dở từ Volume, tái dùng audio/ảnh và các scene clip hợp lệ; không dùng `--long-form-force-new`. Render ghi log từng scene, dùng scale trung gian 2304px đủ cho 1080p. Nếu một visual web không render được trong 2 phút, bot thay bằng visual scene trước và render lại, không gọi thêm TTS hoặc image API. Video dài không được đăng sang Facebook/TikTok và không tạo bản tiếng Việt. Không còn chế độ `prepare/finalize`, không cần Volume để giữ ảnh tạo dần:
