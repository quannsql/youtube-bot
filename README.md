...
Mỗi Short là video SO SÁNH 2 đối tượng, gồm 6 cảnh dựng theo layout cố định (2 ảnh đại diện ở trên + người que hoạt hình chỉ tay sang bên đang được nói tới). Ảnh thật của 2 đối tượng được tìm qua Brave rồi tự động thử Wikimedia Commons nếu Brave không có kết quả phù hợp; bot không dùng GPT Image để giả lập khuôn mặt người nổi tiếng. Nếu vẫn thiếu một trong hai ảnh, lượt chạy dừng trước render/upload thay vì xuất panel màu trống. Các panel dựng bằng Pillow rồi ghép bằng FFmpeg. Video dài (kênh giải thích, hình hoạt hình người que) mặc định dùng 15 visual, tất cả do GPT Image 2 tạo; số lượng chỉnh qua `LONG_FORM_AI_IMAGES` (mặc định 15). Mặc định `BRAVE_WEB_IMAGES_PER_LONG_FORM=0`, chỉ lấy ảnh thật từ nguồn web được phép cho scene mà planner gắn `search_query` là địa danh/công trình/hiện vật thật sự nổi tiếng. Các trang nguồn đã dùng được lưu trong `web_sources.json` và thêm vào mô tả video.
